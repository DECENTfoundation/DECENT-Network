...
cd $PRJ_ROOT_DIR/prj/gui_wallet/lib_gui_wallet_qt
...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
	BINARY_DIR0=$PRJ_ROOT_DIR/sys/`lsb_release -c | cut -f 2`/bin
	BINARY_DIR=$PRJ_ROOT_DIR/sys/`lsb_release -c | cut -f 2`/bin
        #cd ../../../sys/`lsb_release -c | cut -f 2`/bin
elif [[ "$OSTYPE" == "darwin"* ]]; then
	BINARY_DIR0=$PRJ_ROOT_DIR/sys/mac/bin
	BINARY_DIR=$PRJ_ROOT_DIR/sys/mac/bin/gui_wallet.app/Contents/MacOS
