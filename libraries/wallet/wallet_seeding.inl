std::vector<seeder_object> wallet_api::list_seeders_by_price( uint32_t count )const
{
   return my->_remote_db->list_seeders_by_price( count );
}

fc::optional<std::vector<seeder_object>> wallet_api::list_seeders_by_upload( const uint32_t count )const
{
...
std::vector<seeder_object> wallet_api::list_seeders_by_rating( const uint32_t count )const
{
