...
private:
   ...
   int compute_mu(std::fstream& file, unsigned int q, int indices[], element_t v[], element_t mu[]);
   int compute_sigma(element_t *sigmas, unsigned int q, int *indices, element_t *v, element_t &sigma);
   int get_sigma( uint64_t idx, element_t ms[], element_pp_t u_pp[], element_t& pk, element_t out[]);
   ...
   inline int get_m(std::fstream &file, uint32_t i, uint32_t j, element_t& out);
